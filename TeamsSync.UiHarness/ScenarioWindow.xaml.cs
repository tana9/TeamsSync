...
public partial class ScenarioWindow
{
    ...
    public ScenarioWindow(MainWindowViewModel viewModel, DemoTeamsGateway gateway)
    {
        ...
        Scenarios =
        [
            new("初期状態", "チーム選択後、入力前の空状態を確認します。", "demo-team", "", SyncMode.AddOnly),
            new("追加のみ", "既存メンバーを維持し、新規メンバー1名を追加する差分です。", "demo-team",
                "taro@example.com\nhanako@example.com\nlong.user@example.com\nnew.member@example.com", SyncMode.AddOnly),
            new("完全同期・削除あり", "リスト外の一般メンバーが赤い削除行として表示されます。", "demo-team",
                "taro@example.com", SyncMode.FullSync),
            new("指定メンバーを削除", "入力した一般メンバーだけを削除するケースです。", "demo-team",
                "hanako@example.com", SyncMode.RemoveSpecified),
            new("未解決ユーザー", "存在しないメールアドレスによるエラー行と実行不可状態です。", "demo-team",
                "unknown.user@example.com", SyncMode.AddOnly),
            new("長い表示内容", "長いチーム名・表示名を使って折り返しとDPI拡大を確認します。", "long-team",
                "とても長い表示名を持つレイアウト確認用ユーザー <long.user@example.com>\nsecond.new@example.com", SyncMode.AddOnly),
            new("同期成功結果", "追加処理を実行し、成功サマリーと保存ボタンを表示します。", "demo-team",
                "taro@example.com\nhanako@example.com\nlong.user@example.com\nnew.member@example.com", SyncMode.AddOnly, Execute: true),
            new("同期一部失敗", "2件の追加のうち1件を失敗させ、失敗一覧を表示します。", "demo-team",
                "taro@example.com\nhanako@example.com\nlong.user@example.com\nnew.member@example.com\nsecond.new@example.com",
                SyncMode.AddOnly, Execute: true, FailingUserId: "user-second")
        ];
    ...
    private async void ApplyScenario(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            if (string.IsNullOrWhiteSpace(scenario.Input)) return;
            ...
            if (scenario.Execute) await _viewModel.SyncWorkspace.ExecuteSyncCommand.ExecuteAsync(null);
        }
        finally
        {
...
public sealed record DemoScenario(string Name, string Description, string TeamId, string Input,
    SyncMode Mode, bool Execute = false, string? FailingUserId = null);
