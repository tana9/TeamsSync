...
- [ ] `SyncWorkspaceViewModel`から、実行前再検証・同期実行・実行後再取得を調整する処理を専用のコーディネーターへ分離する
- [ ] 保存したCSVを開くOS依存処理を`ISavedFileLauncher`などのサービスへ移し、ViewModelから`Process.Start`への直接依存をなくす
- [ ] プレビュー、部分失敗、キャンセル、再取得失敗、CSV起動失敗の既存動作を維持するテストを追加する
- [ ] 分割後も処理の流れを追いやすくするため、細かすぎるサービス分割や単なる委譲だけの型は増やさない
...
- [ ] 失敗一覧の`ScrollViewer`内`ItemsControl`を仮想化対応の一覧へ変更するか、先頭N件と総件数を表示する方式にする
- [ ] 数百～数千件の失敗結果でも、結果パネルの表示とスクロールが重くならないことを確認する
- [ ] アクションバーの`StatusText`を折り返し表示し、最小ウィンドウ幅や長いエラー文でも同期実行ボタンと重ならないようにする
...
- [ ] ESC処理の優先順位を「ContentDialog、同期キャンセル、Snackbar」の順で判定できるテスト可能な形にする
- [ ] ダイアログ表示中は背面のSnackbarを閉じず、同期中はSnackbarよりキャンセルを優先し、それ以外はSnackbarを閉じることをテストする
- [ ] Snackbarが表示されていない場合はESCを処理済みにせず、他のWPF標準操作を妨げないことをテストする
