...
- [ ] `CsvHelper`の`BadDataFound = null`による構文エラーの黙殺をやめ、引用符崩れなどの不正なCSVを行番号付きエラーとして拒否する
- [ ] 行ごとの列数がヘッダーと不整合な場合に、誤った列をメンバー識別子として採用せず読込を中止する
- [ ] 閉じていない引用符、余分な引用符、列不足、列過多、引用符内改行を含む正常データの境界テストを追加する
- [ ] CSV読込エラー時に以前の同期差分が無効化され、完全同期を実行できないことをViewModelテストで確認する
...
- [ ] 対象者ごとの成功、失敗、未実行を結果画面と監査ログへ記録し、キャンセルまたは部分失敗後はGraph上の最新状態から未反映分だけを再計画する（結果CSVは`result.Operations`だけでなく実行前プラン全体を基準にし、未着手の対象も欠落させない）
