...
- [ ] 結果CSVのファイル名へミリ秒と`ExecutionId`を含め、同一秒・同一チームの連続実行でも既存ログを上書きしない
- [ ] `FileMode.CreateNew`などでファイル名衝突を検出し、衝突時は一意な別名で保存する
- [ ] 結果CSVの既定保存先を`AppContext.BaseDirectory`から、一般ユーザーが書き込める`%LocalAppData%\TeamsSync\Logs`配下へ移し、JSON監査ログと保存場所を統一する
- [ ] 同一秒内の連続保存、同名チームの保存、書き込み権限がない保存先、既存ファイルとの衝突を自動テストする
