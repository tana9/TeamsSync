...
### 同期結果CSVの数式注入を防止する（撤去済み）

- [x] チーム名、メールアドレス、エラー文など外部由来の値が`=`, `+`, `-`, `@`で始まる場合に数式として解釈されない形式へ無害化する処理(`SyncResultWriter.SanitizeFormulaInjection`/`CsvExternal`)を撤去した。本アプリの利用者は自組織のメンバーに限られ、悪意ある改ざんを想定した無害化は過剰と判断し、通常のCSVエスケープ(`Csv`)のみに戻した（利用者確認済み、2026-08-04）
...
- [x] 429を増加させない並列数を決定する（`GET /me/ownedObjects`が所有グループIDを1回で返すことを確認し、
  チーム単位のバッチ取得・キャッシュ・並列数調整（`TeamOwnershipCache`/`TeamMembersBatchFetcher`）を撤去。
  `joinedTeams`との積集合を取るだけの方式へ置き換えたため、チーム数に関わらず常にGraph呼び出し2回で
  完結し、並列数チューニング自体が不要になった）
...
### XLSXの展開処理を追加で制限する（撤去済み）

- [x] 展開後合計サイズ、ZIPエントリー数、単一エントリーサイズ、圧縮率の上限チェック(`MemberFileSecurityValidator.ValidateExcelArchive`)を撤去した。本アプリの利用者は自組織のメンバーに限られ、改ざん・Zip爆弾のような悪意ある入力を想定した検証は過剰と判断し、ファイルサイズ上限(`EnsureFileSizeWithinLimit`)と行数・列数上限のみに委ねる方針とした（利用者確認済み、2026-08-04）
