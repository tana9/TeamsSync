...
### タイムアウトと利用者キャンセルを区別する

- [x] 同期操作中の`OperationCanceledException`を、呼び出し元のキャンセルトークンが要求済みの場合だけ利用者キャンセルとして扱う
- [x] `BusyOperationRunner`へ呼び出し元のキャンセルトークンを渡し、通信タイムアウトなどを通常のエラーとして通知する
- [x] 同期操作と共通UI処理について、タイムアウトと利用者キャンセルを区別する回帰テストを追加する

完了条件: 通信タイムアウトで残りの同期操作を「キャンセル」として中断せず、利用者へ誤ったキャンセル表示を行わない。
