...
Graph APIの読み取り・書き込みは共通のHTTP回復性パイプラインを使用します。読み取り(GET)は408、429、5xx、通信例外、タイムアウトを対象に再試行します。書き込み(POST/PUT/DELETE等)は応答喪失時の二重実行を避けるため、429以外(503・タイムアウト等)は再試行しません。429はGraph側が処理前に明示的に拒否した応答であるため、書き込みでも例外的に再試行します。`Retry-After`がある場合はその待機時間を優先し、全体・試行単位のタイムアウトとサーキットブレーカーも適用します。
